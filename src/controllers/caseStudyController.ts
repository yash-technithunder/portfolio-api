...
const createCaseStudy = async (req: Request, res: Response) => {
    try {
        const files = req?.files as {[fieldname: string]: Express.Multer.File[]} | undefined;
        const findFileByFieldname = (fieldname: string): Express.Multer.File | undefined => (files ? files[fieldname]?.[0] : undefined);
        const coverImageFile = findFileByFieldname('coverImage');
        ...
        const coverImageUrl = coverImageFile ? await uploadImageToCloudinary(coverImageFile) : '';
        ...
        const caseStudyData = {
            ...
            techStack: techStack,
            name: req.body.name,
            coverImage: coverImageUrl, // Add the coverImage URL here
            introduction: {
                descriptions: req.body.introduction.descriptions,
                image: getImgUrl('introduction[image]'),
                industry: req.body.introduction.industry || '',
...
const updateCaseStudy = async (req: Request, res: Response) => {
    try {
        ...
        const coverImageUrl = coverImageFile ? await uploadImageToCloudinary(coverImageFile) : existingCaseStudy.coverImage;
