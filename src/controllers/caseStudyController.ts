...
import { uploadImage } from '../utility/helper';
...
const createCaseStudy = async (req: Request, res: Response) => {
  try {
    const files = req?.files as
      ...
      | undefined;
    const findFileByFieldname = (
      fieldname: string,
    ): Express.Multer.File | undefined =>
      files ? files[fieldname]?.[0] : undefined;
    const images = [
    ...
    const filteredImages: Express.Multer.File[] = (images ?? []).filter(
      isMulterFile,
    ) as Express.Multer.File[];
    const prefix = 'https://api.technithunder.com/';
    const imageUrls = await Promise.all(
      filteredImages.map((image) => ({
        imgUrl: uploadImage(image).replace('/usr/src/app/', prefix),
        fieldname: image.fieldname,
      })),
    );

    const getImgUrl = (field: string) => {
      return imageUrls.filter((i) => i.fieldname === field)[0]?.imgUrl;
    };
  ...
  try {
    ...
    const findFileByFieldname = (
      ...
      return fileArray.find(
        (file: Express.Multer.File) => file.fieldname === fieldname,
      );
    };
    ...
    const imageKeys: Array<keyof CaseStudyDocument> = [
      'backstory',
      'challenge',
      'introduction',
      'result',
    ...
    const introductionDesc =
      req?.body?.introduction?.descriptions ||
      existingCaseStudy.introduction.descriptions;
    const backstoryDesc =
      req?.body?.backstory?.descriptions ||
      existingCaseStudy?.backstory?.descriptions;
    const challengeDesc =
      req?.body?.challenge?.descriptions ||
      existingCaseStudy?.challenge?.descriptions;
    const solutionDesc =
      req?.body?.solution?.descriptions ||
      existingCaseStudy?.solution?.descriptions;
    const resultDesc =
      req?.body?.result?.descriptions || existingCaseStudy?.result?.descriptions;
    ...
    const updatedCaseStudyData = {
      ...
      introduction: {
        ...
        industry:
          req?.body?.introduction?.industry ||
          existingCaseStudy?.introduction?.industry,
        platform:
          req?.body?.introduction?.platform ||
          existingCaseStudy?.introduction?.platform,
        title:
          req?.body?.introduction?.title ||
          existingCaseStudy?.introduction?.title,
      },
      backstory: {
...
const getAllCaseStudies = async (req: Request, res: Response) => {
  try {
    ...
    console.error(
      'Error occurred while fetching all CaseStudies:',
      err.message,
    );
    return res.status(500).json({
      success: false,
